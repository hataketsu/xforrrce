...
<map version="1.10" tiledversion="1.11.0" orientation="orthogonal" renderorder="left-down" width="WIDTH_PLACEHOLDER" height="HEIGHT_PLACEHOLDER" tilewidth="24" tileheight="24" infinite="0" nextlayerid="5" nextobjectid="1">
 ...
 <layer id="1" name="grass" width="WIDTH_PLACEHOLDER" height="HEIGHT_PLACEHOLDER">
  <data encoding="csv">
  DATA_PLACEHOLDER
</data>
 </layer>
...
def convert_map_to_tmx( map_index):
    ...
    for c in data:
        ...
        buf.append(str(c))
    data = ','.join(buf)
    output = template.replace('WIDTH_PLACEHOLDER', str(width)).replace('HEIGHT_PLACEHOLDER', str(height)).replace('DATA_PLACEHOLDER', data).replace('TILE_PLACEHOLDER', str(tile_map))
