...
class Sentry(Sprite):
    ...
    def update(self):
        ...
        if self.hp <= 0:
            ...
            self.window.add_message( self.window.tank.center_x, self.window.tank.center_y, "+10 EXP")
            self.hp = -1
    ...
    def fire(self):
        ...
        if distance_to_tank < 160 and self.cooldown <= 0:
            ...
            bullet_speed = 2
            bullet.change_x = (self.window.tank.center_x - self.center_x) / distance_to_tank * bullet_speed
            bullet.change_y = (self.window.tank.center_y - self.center_y) / distance_to_tank * bullet_speed
